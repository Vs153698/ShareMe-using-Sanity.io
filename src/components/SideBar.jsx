      ...
      <div className="flex flex-col">
          ...
          <div className="flex flex-col gap-5">
              ...
              {categories.slice(0,categories.length -1).map((category)=>(
                    <NavLink onClick={handleclosesiderbar} to={`/category/${category.name}`} key={category.name} className={({isActive})=> isActive ? isActiveStyle : isNotActiveStyle}>{category.name}</NavLink>
              ))}

          </div>
      </div>
    {user && (
        <Link onClick={handleclosesiderbar} to={`user-profile/${user._id}`} className="flex my-5 mb-3 gap-2 p-2 items-center bg-white rounded-lg shadow-lg mx-3"><img src={user.image} className="w-10 h-10 rounded-full" alt="" /><p>{user.username}</p></Link>
    )}
  </div>;
};
